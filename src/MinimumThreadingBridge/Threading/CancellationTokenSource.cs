...
namespace System.Threading
{
    ...
    public class CancellationTokenSource : IDisposable
    {
        ...
        private volatile object[] m_registeredCallbacksLists;
        ...
        private int CallbackCount
        {
            get
            {
                object[] callbackLists = m_registeredCallbacksLists;
                if (callbackLists == null)
                    return 0;

                int count = 0;
                foreach (SparselyPopulatedArray<CancellationCallbackInfo> sparseArray in callbackLists.Cast<SparselyPopulatedArray<CancellationCallbackInfo>>())
                {
                    if (sparseArray != null)
                    {
            ...
            Action<object> callback, object stateForCallback, SynchronizationContext targetSyncContext, ExecutionContext executionContext)
        {
            ...
            if (!IsCancellationRequested)
            {
                ...
                object[] registeredCallbacksLists = m_registeredCallbacksLists;
                if (registeredCallbacksLists == null)
                {
                    object[] list = new object[s_nLists];
                ...
                var callbacks = (SparselyPopulatedArray<CancellationCallbackInfo>)Thread.VolatileRead(ref registeredCallbacksLists[myIndex]);
                if (callbacks == null)
                {
                    ...
                    callbacks = (SparselyPopulatedArray<CancellationCallbackInfo>)registeredCallbacksLists[myIndex];
                }
        ...
        private void ExecuteCallbackHandlers(bool throwOnFirstException)
        {
            ...
            try
            {
                ...
                {
                    SparselyPopulatedArray<CancellationCallbackInfo> list = (SparselyPopulatedArray<CancellationCallbackInfo>)Thread.VolatileRead(ref callbackLists[index]);
                    if (list != null)
                    {
